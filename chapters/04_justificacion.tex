...
Plataformas como Home Assistant, Nextcloud, Immich o servicios equivalentes permiten recuperar control sobre datos y automatizaciones. Su debilidad no es la privacidad, sino la fragmentación. Son piezas sólidas, pero requieren una capa de coordinación que interprete contexto, conecte eventos, active flujos y permita interactuar mediante lenguaje natural.

\section{Servicios especializados frente a una plataforma monolítica}
Se valoró concentrar todas las capacidades en una única aplicación. Esa alternativa reducía inicialmente el número de componentes, pero aumentaba el acoplamiento, dificultaba sustituir tecnologías y convertía cualquier fallo en una interrupción general. La arquitectura final asigna cada responsabilidad a un servicio especializado: Home Assistant para domótica, Frigate para vídeo, Immich para fotografías, Mealie para recetas, Nextcloud para documentos, Ollama para inferencia y n8n para automatización.

Este enfoque exige mayor disciplina de red, autenticación y observabilidad, pero permite actualizar, reiniciar o reemplazar un componente sin rediseñar el conjunto y tener una solución infinitamente mejor que si se intentara desarrollar cada app por separado. La interfaz conversacional funciona como capa de acceso común y no como propietaria de los datos.
...
n8n permite conectar eventos y servicios mediante flujos visuales y programables \cite{n8nDocs}. En Odín, esta capa actúa como sistema nervioso: recibe entradas, decide rutas, invoca modelos, transforma datos, almacena resultados y dispara acciones. Por eso la memoria dedicará una sección específica a los flujos diseñados, sus nodos principales, errores encontrados y criterios de robustez.
